...
class Resources
{
    ...
    public function getResources(Request $request, $input_string)
    {
        $data = $request->all();
        if (isset($data['limit'])) {
            $limit = $data['limit'];
        ...
        $total_entries = DB::table('Resources')
            ->where('subject_name', '=', $input_string)
            ->count();
        ...
        $resources = array("resources" => $data, "next_url" => $next_url, "prev_url" => $prev_url);
        return $this->json_generic($resources);
    }
}
